...
The adapter exposes actions only when routes middleware, endpoint metadata, and explicit registration are all present. Versioned non-enumerable descriptors use `Symbol.for`, not a global registry. Route tables compile once and reject ambiguous templates. HTTP input uses bounded JSON reads and explicit origin checks; application middleware owns authentication and authorization. The optional OpenAPI entry converts Standard JSON Schema resources without executing actions. Production route and hook canaries live in `apps/playground/e2e/routes.spec.ts`; run them after both Turbopack and Webpack builds.
