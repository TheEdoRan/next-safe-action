...
With the `useAction` hook (and `useOptimisticAction`, `useStateAction`, `useOptimisticStateAction`), the return object is itself a **discriminated union keyed on `status`** and shorthand booleans. Checking any discriminant narrows `result` to the matching branch:
