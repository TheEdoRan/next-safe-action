...
- The value should conform to the client's `ServerError` type, which is inferred from `handleServerError`'s return type. Declare a union there (e.g. `{ code: "INTERNAL" | "OUT_OF_STOCK"; message: string }`) to get a fully typed `result.serverError` on the client.
