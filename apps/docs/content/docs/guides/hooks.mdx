...
| `reset` | `() => void` | Reset to initial state, discarding any in-flight execution. On the queued hooks it also skips the dispatches still waiting in the queue, so their writes never run |
...
This applies to all hooks: `useAction`, `useOptimisticAction`, `useStateAction`, and `useOptimisticStateAction`. The `result` field itself is also a discriminated union (see [Action result](/docs/concepts/action-result)), so you get two layers of narrowing: status-level and result-level.
...
The value is captured **once at mount**, like React's `useActionState` initial state: later changes to the option are ignored, and `reset()` restores the mount value. Available on `useAction`, `useOptimisticAction`, `useStateAction`, and `useOptimisticStateAction`. See the [hooks API reference](/docs/api/hooks-api) for details.
...
| Optimistic variant | [`useOptimisticAction`](/docs/guides/optimistic-updates) | [`useOptimisticStateAction`](/docs/guides/coordinating-mutations) |
...
<Cards>
	...
	<Card href="/docs/guides/coordinating-mutations" title="Coordinating mutations">
		Use useOptimisticStateAction to queue overlapping writes instead of racing them.
	</Card>
