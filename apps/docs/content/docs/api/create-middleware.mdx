...
The callback runs once for each completed action definition, not on invocation. It runs for both `action()` and `stateAction()`. Reusing the same middleware in a chain does not call its declaration callback twice.

The exported `ActionDefinition` type contains `action`, `stateful`, `metadata`, `inputSchema`, `outputSchema`, `dynamicInputSchema`, and `bindArgsCount`. The description is shallow-frozen. It exposes no implementation, context, error handler, or middleware stack. Metadata is the declared, unparsed value. Schemas and metadata are references, not deep copies; do not modify them.
