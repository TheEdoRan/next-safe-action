...
| `utils`            | `{ currentState: State; updateFn: (state: State, input: InferIn<S>) => State }` `&` [`HookCallbacks`](/docs/types#hookcallbacks)                            | Object with required `currentState`, `updateFn` and optional base utils and callbacks. See below for more information.                                                                                                                                              |
