...
const features: { title: string; description: string }[] = [
	...
	},
	{
		...
		description: "Decide how to return execution errors to the client and how to log them on the server.",
	},
	{
			...
			<div className="mx-auto w-full max-w-4xl">
				<div className="flex-col flex gap-y-20 max-[479px]:gap-[60px] items-center lg:items-center py-20 lg:py-24">
					<div className="flex-col flex items-center justify-center gap-y-[60px] max-[479px]:gap-[60px]">
						<div className="text-center font-bold text-3xl sm:text-4xl lg:text-5xl">Why choose next-safe-action?</div>
						<div className="min-w-full flex flex-col space-y-6">
							{features.map(({ title, description }, idx) => (
								<Fragment key={idx}>
									{idx > 0 ? <div className="h-px min-h-[1px] min-w-full bg-zinc-200 dark:bg-zinc-700"></div> : null}
									<div className="flex space-x-2 sm:space-x-4 md:space-x-6">
										...
										<div className="flex-col flex items-start gap-y-2 max-[479px]:pr-10">
											<div className="font-bold text-xl sm:text-xl">{title}</div>
											<div
